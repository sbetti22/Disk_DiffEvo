...
def initialize_diskfm():
    ...
    model_here_convolved = fits.getdata(DISKFM_DIR + '/initialize_files/' + FILE_PREFIX + '_FirstModelConvolved.fits')
    pad = (REDUCED_DATA.shape[0]//2) - (model_here_convolved.shape[0]//2)
    ...
    diskobj = DiskFM(None,
                     ...
                     basis_filename=DISKFM_DIR + '/initialize_files/' + FILE_PREFIX + '_klbasis.h5',
...
with open('de_parameter_file.yaml', 'r') as yaml_file:
...
INITIAL_PARA = params_de_yaml['INITIAL_PARA'] #  

WAVELENGTH = params_de_yaml['WAVELENGTH'] # um
...
print(f'deleting {len(csv_fils)} .csv files')
for mod in csv_fils:
    os.remove(mod)

mcfost_fils  = glob.glob(SAVE_DIR + '/*')
print(f'deleting {len(mcfost_fils)} mcfost files')
