...
#
        ...
        for idensity in range(self.nzones):
            ...
            # print(self.density_zones[idensity]['dust_nspecies'])
...
#Grain properties
  {zone[dust_nspecies]:<2d}                      Number of species""".format(zone=self.density_zones[0])
        ...
        template+="""
  {dust[grain_type]} {dust[ncomponents]:<2d} {dust[mixing_rule]:<1d} {dust[porosity]:<5.2f} {dust[mass_fraction]:<5.2f} {dust[vmax]:<5.2f}     Grain type (Mie or DHS), N_components, mixing rule (1 = EMT or 2 = coating),  porosity, mass fraction, Vmax (for DHS)
  ...
  {star[fUV]:<5.3f}  {star[slope_fUV]:<5.3f}     fUV, slope_fUV """.format(star=recarray2dict(star), str_is_blackbody = 'T' if star['l_is_blackbody'] else 'F')
    ...
    def set_parameter(self, paramname, value):
        ...
        # modify this or write your own code...
            ...
            self.RT_imin = value
            self.RT_imax = value
        elif paramname == 'disk_pa':
            self.disk_pa = value
        elif paramname == 'ncomponents':
            self.density_zones[0]['dust'][0]['ncomponents']= value
        elif paramname == 'ice_species':
            self.density_zones[0]['dust'][1]['filename']= value
        elif paramname == 'alpha_viscosity':
            self.alpha_viscosity = value
        elif paramname == 'vmax':
            self.vmax = value
