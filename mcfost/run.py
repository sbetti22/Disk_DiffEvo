...
def run_one_file(filename, wavelengths=[], options=None, move_to_subdir=True, delete_previous=True):
    ...
    if move_to_subdir:
        ...
        if delete_previous and os.path.isdir(modeldir + '/' + modelname):
            _log.debug("Removing previous calculated sub-folders.")
            import shutil
            shutil.rmtree(modeldir + '/' + modelname, ignore_errors=True)

        os.mkdir(modeldir + '/' + modelname)
            
        subprocess.call('chmod -R g+w '+modeldir + '/' + modelname,shell=True)
        subprocess.call('mv '+filename+' '+modeldir + '/' + modelname,shell=True)

        filename = os.path.join(modeldir + '/' + modelname, os.path.basename(filename))
    ...
    for wl in wavelengths:
        run_image(filename, wl, options=options, delete_previous=delete_previous)
...
def run_image(filename, wavelength, options=None, raytrace=True,  delete_previous=True, *args):
    ...
    if options== None:
        ...
        cmdstr = 'mcfost '+os.path.basename(filename)+' -img '+str(wavelength) + ' ' + options
    if raytrace: cmdstr += " -rt"
...
def grid_generator(base_paramfile, paramsdict, outfile, filename_prefix=None, start_counter=1):
...
#    counter = start_counter
    ...
    for params in params_generator:
        ...
        output_fn = f"{filename_prefix}_{start_counter}.para"#.format(filename_prefix, counter)
        base_paramfile.writeto(outfile + output_fn)
